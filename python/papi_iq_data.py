...
#-------------------------------------------------------------------------------
def unpack_raw12(byte_buffer, a_len):
    """
    ...
    """
    total_iq_data = a_len
    expected_bytes = (total_iq_data * 3) // 2
    ...
    b0 = raw_bytes[:, 0].astype(np.int16)
    b1 = raw_bytes[:, 1].astype(np.int16)
    b2 = raw_bytes[:, 2].astype(np.int16)
    ...
    unpacked_data = np.empty(total_iq_data, dtype=np.int16)
    # Extend sign bit
    unpacked_data[0::2] = (pixel_0 << 4).astype(np.int16) >> 4
    unpacked_data[1::2] = (pixel_1 << 4).astype(np.int16) >> 4
    # Reshape to 4 row matrix
    return unpacked_data.reshape(-1, 4).T
#-------------------------------------------------------------------------------
...
def get_data(a_datas, a_len, a_atom_size, a_data_type):
    ...
    unpacked_data = unpack_raw12(raw_data, ((a_len//3)*4)*4)
...
def main(argv):
    ...
    # values[2] = Q1
    # values[3] = I1
    plot_len = min(max_plot_len, len(data_line.values[0]))
    plot_sig("Chn0 QI", data_line.values[0][:plot_len-1], data_line.values[1][:plot_len-1])
    plot_sig("Chn1 QI", data_line.values[2][:plot_len-1], data_line.values[3][:plot_len-1])

    #---------------------------------------------------------------------------

    # Remove DC Bias Comented out
    i_clean = data_line.values[1][:65536] # - np.mean(data_line.values[2][:2048])
    q_clean = data_line.values[0][:65536] # - np.mean(data_line.values[3][:2048])
    complex_signal = i_clean.astype(np.float32) + 1j * q_clean.astype(np.float32)
    windowed_signal = complex_signal * np.hamming(65536)
    fft_result = np.fft.fft(windowed_signal)
    fft_shifted = np.fft.fftshift(fft_result)
    magnitude_db = 20 * np.log10(np.abs(fft_shifted) + 1e-9)

    freqs = np.fft.fftshift(np.fft.fftfreq(65536, d=1/g_fs_freq))
    plot_fft("FFT Chn0", freqs, magnitude_db)
    
    i_clean = data_line.values[3][:2048] # - np.mean(data_line.values[2][:2048])
    q_clean = data_line.values[2][:2048] # - np.mean(data_line.values[3][:2048])
    complex_signal = i_clean.astype(np.float32) + 1j * q_clean.astype(np.float32)
    windowed_signal = complex_signal * np.hamming(2048)
    fft_result = np.fft.fft(windowed_signal)
    fft_shifted = np.fft.fftshift(fft_result)
    magnitude_db = 20 * np.log10(np.abs(fft_shifted) + 1e-9)

    freqs = np.fft.fftshift(np.fft.fftfreq(2048, d=1/g_fs_freq))
    plot_fft("FFT Chn1", freqs, magnitude_db)
